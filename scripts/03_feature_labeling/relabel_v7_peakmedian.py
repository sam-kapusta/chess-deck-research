#!/usr/bin/env python3
"""Feature relabeler — v7: PEAK + MEDIAN boards (fixes piece over-specification).

Labels from BOTH the peak (top-10 strongest) AND median (typical, p40-60) boards, and shows v6's
peak-only label as context. Reason: labeling from the peak alone over-specifies the piece — the
peak boards are piece-homogeneous (f103's peak = queen knight-forks) while the median fires on
rook/king forks too. Feeding median boards + telling the model "peak said queen, but generalize if
the median shows varied pieces" produces the true label ("Knight fork wins major piece"). Reads
peak_median_profiles_d2048_k6.json from build_peak_median_profiles.py. Same opus-4-8 xhigh engine.
...
def board_line(i, ex, op):
    ...
    if not isinstance(a, dict):
        return None
    return (f"{i}. [{a.get('tactical_motif','?')}] played {ex['uci']}\n"
            f"   WHAT WENT WRONG: {(a.get('blunder_summary') or '')[:240]}\n"
            f"   HOW IT'S PUNISHED (refutation): {(a.get('refutation_analysis') or '')[:300]}\n"
            f"   BEST MOVE: {(a.get('best_moves_analysis') or '')[:150]}\n"
            f"   INTENT: {(a.get('move_intent') or '')[:100]}")

def build(fid, prof_f, op, st):
    """v7: prof_f is {peak:[...], median:[...]}. Show both bands so the label reflects the feature's
    full range, not just the piece-homogeneous peak."""
    s = st.get("f" + fid) or st.get(str(fid)) or {}
    prev = PREV.get(fid, {})
    peak = [l for i, ex in enumerate(prof_f.get("peak", [])[:10], 1) for l in [board_line(i, ex, op)] if l]
    med = [l for i, ex in enumerate(prof_f.get("median", [])[:10], 1) for l in [board_line(i, ex, op)] if l]
    if len(peak) + len(med) < MIN_BOARDS:
        return None
    prevblock = ""
    if prev.get("chip"):
        prevblock = f'Current label: "{prev.get("chip")}" — {prev.get("label","")}\n\n'
    body = ""

#!/usr/bin/env python3
"""All-fields feature relabeler — v4: adds REFUTATION line + RESULT-framed mistakes.

v4 over v3: feeds `refutation_analysis` (the punishing line — previously UNUSED) and lengthens the
truncated summaries (170->320). Reason (f882): the labeler only saw "queen is hanging" (setup) and
stopped before "the trade loses material" (result), because blunder_summary was cut at 170 chars and
refutation_analysis was never passed. The mistake lives in the RESULT of the forcing sequence, not the
setup move. v4 prompt names the result-framed mistake (e.g. "Queen trade loses material") not the
shallow setup ("Hangs queen"), and never names the corrective move.
...
def build(fid, prof_f, op, st):
    ...
    for i, ex in enumerate(prof_f.get("examples", [])[:10]):
        ...
        if isinstance(a, dict):
            lines.append(
                ...
                f"   WHAT WENT WRONG: {(a.get('blunder_summary') or '')[:320]}\n"
                f"   HOW IT'S PUNISHED (refutation line): {(a.get('refutation_analysis') or '')[:320]}\n"
    ...
    return ("Name this sparse-autoencoder chess feature: the ONE recurring mistake across its top-10 positions.\n"
            "CRITICAL: name the mistake by its RESULT — what actually goes wrong once the forcing sequence plays "
            "out (see the 'HOW IT'S PUNISHED' refutation line), not just the surface setup. Example: if the player "
            "enters a queen trade / forcing sequence that leaves them down material after the dust settles, the "
            "mistake is 'Queen trade loses material' — NOT the shallow 'Hangs queen' (which only describes the setup). "
            "Read the refutation line on each board to see the true cost; the recurring mistake often lives in the "
            "RESULTING position, not the move itself.\n"
            "Decide DIRECTION from evidence: a move that loses the player's own material (incl. via the refutation "
            "sequence) is a SELF-INFLICTED blunder; a materially-safe move that skips a win is an OMISSION ('Missed X'). "
            "Label what the player DID WRONG, never the corrective move (NOT 'Relocate queen to safety' — that's the fix).\n"
            "NAME THE MECHANISM: include the specific recurring detail (which piece, trade-vs-hang, the 2-for-1, "
            "a square/file, opening vs endgame) when the boards share one. Fall back to generic only if they truly don't.\n\n"
            + seeblock(s) + "\n\nPER-POSITION ANALYSES (top 10):\n" + chr(10).join(lines) +
            '\n\nJSON: {"chip":"<3-6 words; the RESULT-framed mistake>","mistake_type":"<missed_win|hung_own|greedy|trade|positional|endgame>",'

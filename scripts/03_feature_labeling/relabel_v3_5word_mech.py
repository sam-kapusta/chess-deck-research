...
def build(fid, prof_f, op, st):
    ...
    return ("Name this sparse-autoencoder chess feature: the ONE recurring mistake across its top-10 positions.\n"
            "CRITICAL: decide the mistake's DIRECTION from the evidence, not from any default. "
            "A move that loses the player's OWN material (see 'player hung own piece %' — high means the played move itself "
            "drops a piece) is a SELF-INFLICTED blunder; name it for what was hung ('Hangs X'), even if a stronger move also existed. "
            "A move that is materially SAFE but skips a winning capture/tactic (high best-move-wins-material, low hung-own) is an "
            "OMISSION; name it 'Missed X'. When BOTH are clearly true across the boards, name the DOMINANT pattern and lower the "
            "consistency score. Trust the per-board BEST MOVE analyses for traps/multi-move tactics SEE can't see.\n"
            "NAME THE MECHANISM: if the boards share a specific recurring detail (which enemy piece captures, whether it hangs "
            "while giving check, a promoted queen, a specific square/file, opening vs endgame), put it in the chip. Only fall "
            "back to a generic chip if the 10 boards genuinely share no finer mechanism.\n\n"
            + seeblock(s) + "\n\nPER-POSITION ANALYSES (top 10):\n" + chr(10).join(lines) +
            '\n\nJSON: {"chip":"<3-5 words; include the mechanism>","mistake_type":"<missed_win|hung_own|greedy|trade|positional|endgame>",'

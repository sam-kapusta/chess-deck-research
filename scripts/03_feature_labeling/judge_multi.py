...
Judge from the boards what the positions ACTUALLY share. Then:
  ...
  "tagger_coverage": one of
    "covered"      — the true concept is NAMED (right + deep enough) by at LEAST ONE of the tagger's tags
                     (not necessarily the highest-voted one — ANY tag that appears in the list counts).
...
def build_prompt(fi,positions,tagger_votes,alt_label):
    lines=[f"SAE FEATURE {fi}",
           f"TAGGER TAGS (vote count across top-firing positions, highest first):",
           "  "+", ".join(f"{t} ({c})" for t,c in tagger_votes.items()),
           f"ALT label: {alt_label}","","TOP-FIRING POSITIONS:"]
...
def main():
    ...
    ap.add_argument("--out",default=f"{BASE}/jr_canon_out/judge_multi.json")
    args=ap.parse_args()
    ...
    def work(fi):
        v=R[str(fi)]
        out=call(bedrock,build_prompt(fi,positions_for(fi),v.get("tagger_votes",{}),opus_labels.get(fi,{}).get("good_label","")))
        out["feature_idx"]=fi; out["tagger_votes"]=v.get("tagger_votes",{}); out["opus_label"]=opus_labels.get(fi,{}).get("good_label","")
        return out
    res=[]

"""Per (family, color, band) opening blunder rates from the Lichess rapid corpus.

Family = Opening column before ':'. For every Opening+Middlegame move (piece-count phase
rule matching the tagger), count moves; if the move lost >= MIN_LOSS cp (from %eval), count
a blunder. Banded by the MOVER's Elo. One scan; numerator and denominator share the population.

Output: opening_rates.json = {"White": {family: {band: {moves, blunders}}}, "Black": {...}}
...
# stats[color][family][band] = [moves, blunders]
stats = {"White": defaultdict(lambda: defaultdict(lambda: [0, 0])),
         "Black": defaultdict(lambda: defaultdict(lambda: [0, 0]))}
...
for fi, f in enumerate(files):
    ...
    try:
        ...
        t = pq.read_table(p, columns=['WhiteElo','BlackElo','TimeControl','movetext','Opening'])
    except Exception as e:
        print('skip', str(e)[:60], flush=True); continue
    cols = [t.column(c).to_pylist() for c in ['WhiteElo','BlackElo','TimeControl','movetext','Opening']]
    for we, be, tc, mt, ope in zip(*cols):
        ng += 1

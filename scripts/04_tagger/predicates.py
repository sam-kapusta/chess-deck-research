...
def missed_prophylaxis(m):
    ...
    try:
        ...
        return []
    # The prevented threat must be a NON-CAPTURE positional move (a plan: break, outpost, attack). If
    # the "threat" is a CAPTURE, the best move is just coincidentally controlling the square where the
    # opponent grabs a hung piece — a hanging-piece hijack (measured: 65% of capture-threat fires are
    # Opus motif=hanging_piece), NOT prophylaxis. Concept gate, not a rate cap. (#57, Sam: judge on the

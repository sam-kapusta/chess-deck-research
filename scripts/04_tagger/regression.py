...
def run():
    ...
    hung_cases = [
        ...
        # Queen captured (Rxd3) BUT mover recaptures a rook (Rxe8+) → net=1. The mover lost a QUEEN and
        # got a rook+knight back — a bigger piece traded down for less. That's "Lost Queen in Exchange"
        # (not Hung Queen: net is small, real compensation came back; not even trade: a queen is bigger
        # than what the opponent lost). (2026-07-17, #58 + Lost-in-Exchange tier.)
        ("queen taken but rook recaptured (net=1) = Lost Queen in Exchange",
         "2krr3/ppp2ppp/8/2q5/3n4/2P2B1P/PPQ2PP1/R3R1K1 w - - 1 18",
         "c2d3", ["Ne2+", "Rxe2", "Rxd3", "Rxe8+", "Kd7", "Rae1"], "Lost Queen in Exchange"),
    ...
    grab_cases = [
        ...
        ("pawn break NEG: pawn capture is not a break", PR.missed_pawn_break,
         "4k3/8/8/3p4/4P3/8/8/4K3 w - - 0 1", "e1e2", "e4d5", "exd5", [], 120, None),

...
CACHE=BASE+"/cache/maia3_l2l7_concat.pt"
OUT=BASE+"/output/maia3_sae/maia3_l2l7_2048_k32.pt"
    ...
    def forward(self,x):
        ...
        if self.training and self.k_aux>0:
            ...
            if dead.sum()>0:
                ...
                da=torch.zeros_like(dp).scatter(-1,torch.topk(dp,k=ka,dim=-1).indices,
                                                torch.topk(dp,k=ka,dim=-1).values)
...
print(f"Training {D_HIDDEN}d k={K} on {D_IN}d input...")
for ep in range(EPOCHS):
    sae.train(); perm=torch.randperm(len(x_train),device=device); ep_loss=0; nb=0
    for i in range(0,len(x_train),BS):
        loss,_=sae(x_train[perm[i:i+BS]])
        opt.zero_grad(); loss.backward(); opt.step(); sae.norm_dec()
        ep_loss+=loss.item(); nb+=1
    if (ep+1)%20==0 or ep==0:
        sae.eval()
        with torch.no_grad(): val_loss,acts=sae(x_val)
        dead=(sae.dead_cnt>=sae.n_dead).sum().item()
        print(f"  ep {ep+1:>3}: train={ep_loss/nb:.4f} val={val_loss:.4f} dead={dead} ({time.time()-t0:.0f}s)",flush=True)
sae.eval()
with torch.no_grad(): _,all_acts=sae(x)
fr=(all_acts.cpu()>0).float().mean(0).numpy()
print(f"Live: {(fr>0.001).sum()}/{D_HIDDEN}")
torch.save({"state_dict":{k:v.cpu() for k,v in sae.state_dict().items()},
            "config":{"d_input":D_IN,"dict_size":D_HIDDEN,"k":K,
                      "construction":"l2l7_concat","n_train":len(x_train)},

#!/usr/bin/env python3
"""Sweep: k=32 with aux loss at 2048 and 4096 dict sizes. Uses cached activations."""
...
def train_sae(flat, dict_size, k, epochs=5, batch_size=2048, lr=1e-3):
    ...
    for ep in range(epochs):
        ...
        print(
            f"  ep{ep} mse={total_mse/nb:.6f} aux={total_aux/nb:.6f} dead={nd}/{dict_size}"
        )
        sys.stdout.flush()
...
configs = [(2048, 32), (4096, 32)]
results = []
for ds, k in configs:
    print(f"\nTraining dict={ds} k={k} aux=1/32...")
    sys.stdout.flush()
    r = train_sae(flat, ds, k)
    results.append(r)
    print(
        f"  -> dead={r['dead']}/{ds} active={r['active']} L0={r['l0']} FVU={r['fvu']} c_dec={r['c_dec']} ({r['time']}s)"
    )
    sys.stdout.flush()

print("\n" + "=" * 60)
print("K=32 AUX SWEEP RESULTS")
print(
    f"{'dict':>5} {'k':>4}  {'dead':>6}  {'active':>6}  {'L0':>5}  {'FVU':>6}  {'c_dec':>8}"
)
for r in results:
    print(
        f"{r['dict']:>5} {r['k']:>4}  {r['dead']:>6}  {r['active']:>6}  {r['l0']:>5}  {r['fvu']:>.4f}  {r['c_dec']:>.6f}"
    )

# Reference baselines
print("\nBaselines:")
print(
    "  2048 k=32 no-aux:  dead=1161 active=887  L0=32.0 FVU=0.1122 c_dec=0.051876"
)
print(
    "  2048 k=64 aux:     dead=213  active=1835 L0=64.0 FVU=~0.082 c_dec=0.035895"
)
print(
    "  4096 k=64 aux:     dead=1079 active=3017 L0=64.0 FVU=0.0923 c_dec=0.034822"
)

# Save results JSON
results_path = f"{OUTPUT}/k32_aux_sweep_results.json"
